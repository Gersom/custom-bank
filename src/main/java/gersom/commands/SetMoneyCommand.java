...
public class SetMoneyCommand {
    ...
    public static void commandLogic(CustomBank plugin, CommandSender sender, String[] args) {
        ...
        // Establecer el dinero usando Vault
        Economy econ = CustomBank.getEconomy();
        double currentBalance = econ.getBalance(targetPlayer);
        
        // Primero retirar todo el dinero actual
        if (currentBalance > 0) {
            econ.withdrawPlayer(targetPlayer, currentBalance);
        }
        
        // Luego depositar la nueva cantidad
        econ.depositPlayer(targetPlayer, amount);

        // Enviar mensajes de confirmación
        String message = plugin.getConfigs().getLangSetMoney();
